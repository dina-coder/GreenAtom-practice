...
const resources = {
	...
	get_plans_worker_sql: 'select plans.id as plan_id, users.name as name, worker_id, positions.name as position, DATE_FORMAT(date_creation, "%Y-%m-%d") as date_creation, super_id, hr_id, steps.name as step, DATE_FORMAT(date_start, "%Y-%m-%d") as date_start, DATE_FORMAT(date_end, "%Y-%m-%d") as date_end, result, comment, grades.name as grade from users left join plans on plans.worker_id = users.id left join grades on grades.id = plans.grade_id left join positions on positions.id = plans.position_id left join steps on steps.id = plans.step_id where users.id = ?',
	get_user_name_sql: 'select name from users where id = ?',
	get_tasks_sql: 'select id, name, DATE_FORMAT(date_creation, "%Y-%m-%d") as date_creation, content, DATE_FORMAT(date_start, "%Y-%m-%d") as date_start, DATE_FORMAT(date_end, "%Y-%m-%d") as date_end, result from tasks where plan_id = ?',
	get_plans_super_sql: 'select plans.id, users.name as name, positions.id as position_id, positions.name as position, grades.name as grade,  worker_id, DATE_FORMAT(date_creation, "%Y-%m-%d") as date_creation, super_id, hr_id, step_id, steps.name as step, DATE_FORMAT(date_start, "%Y-%m-%d") as date_start, DATE_FORMAT(date_end, "%Y-%m-%d") as date_end, result, grade_id, comment  from plans left join users on users.id=plans.worker_id left join grades on grades.id = plans.grade_id left join positions on positions.id = plans.position_id left join steps on steps.id = plans.step_id where super_id = ?',
